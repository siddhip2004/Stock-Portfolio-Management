...
def fetch_financial_data(tickers, start_date, end_date):
    try:
        # We STOP passing a session here. 
        # yfinance will now use its internal 'curl_cffi' logic automatically.
        data = yf.download(
            tickers, 
            start=start_date, 
            end=end_date, 
            progress=False,
            auto_adjust=True,
            multi_level_index=False # Keeps the dataframe simpler to manage
        )
        ...
        # Logic to get the Close prices
        if isinstance(data.columns, pd.MultiIndex):
            stocks_df = data['Close']
        else:
            # If only one ticker, yfinance might return a standard Index
            stocks_df = data[['Close']] if 'Close' in data.columns else data
...
def optimize_portfolio(stocks_df):
    try:
        ...
        return weights, float(ret), float(vol), float(sharpe)
    except Exception as e:
...
@app.post("/optimize-portfolio/")
async def optimize_portfolio_endpoint(request: PortfolioRequest):
    tickers = [ticker.strip().upper() for ticker in request.tickers.split(',')]
    
    try:
        # Convert YYYY/MM/DD to YYYY-MM-DD
        start_dt = datetime.strptime(request.start_date, '%Y/%m/%d').strftime('%Y-%m-%d')
        end_dt = datetime.strptime(request.end_date, '%Y/%m/%d').strftime('%Y-%m-%d')
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid date format. Use YYYY/MM/DD.")
    
    stocks_df = fetch_financial_data(tickers, start_dt, end_dt)
    
    if stocks_df is None or stocks_df.empty:
        raise HTTPException(status_code=404, detail="No data found. Check ticker symbols and dates.")
    
    weights, ret, vol, sharpe = optimize_portfolio(stocks_df)
    ...
    return {
        "weights": {ticker: f"{weight:.2%}" for ticker, weight in weights.items()},
        "expected_annual_return": f"{ret:.2%}",
