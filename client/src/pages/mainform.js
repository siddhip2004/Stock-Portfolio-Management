...
const Mainform = () => {
  ...
  return (
  <div className="mainform-page-unique-root">
    <div className="optimizer-container">
      <h1 className="optimizer-heading">Optimize Your Portfolio</h1>
      <form onSubmit={handleSubmit}>
        <div className="form-group">
          <label className="optimizer-label">Tickers (comma-separated)</label>
          <input
            type="text"
            value={tickers}
            onChange={(e) => setTickers(e.target.value)}
        ...
        <div className="form-group">
          <label className="optimizer-label">Start Date (YYYY/MM/DD)</label>
          <input
            type="text"
            value={startDate}
            onChange={(e) => setStartDate(e.target.value)}
        ...
        <div className="form-group">
          <label className="optimizer-label">End Date (YYYY/MM/DD)</label>
          <input
            type="text"
            value={endDate}
            onChange={(e) => setEndDate(e.target.value)}
      ...
      {result && (
        <div className="result-section">
          <h2 className="resultHeading">Optimization Results</h2>
          <div className="metrics-grid">
            <div className="metric-card">
              <span className="metric-label">Expected Return</span>
              <span className="metric-value highlight-green">{result.expected_annual_return}</span>
            </div>
            <div className="metric-card">
              <span className="metric-label">Annual Volatility</span>
              <span className="metric-value">{result.annual_volatility}</span>
            </div>
            <div className="metric-card">
              <span className="metric-label">Sharpe Ratio</span>
              <span className="metric-value highlight-blue">{result.sharpe_ratio}</span>
            </div>
          </div>
          ...
          <div className="weights-container">
            ...
            {Object.entries(result.weights).map(([ticker, weight]) => (
              <div key={ticker} className="weight-row">
                ...
                <span className="weight-percentage">{weight}</span>
              </div>
            ))}
          </div>
        </div>
