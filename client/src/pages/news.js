...
const News = () => {
  ...
  const apiKey = "https://newsapi.org/v2/everything?q=tesla&from=2026-03-16&sortBy=publishedAt&apiKey=9c6ef9501ade47ce97ed7d3eb48ddfe5&q=india";

  const fetchData = async () => {
    console.log("API URL:", process.env.REACT_APP_API_KEY_FOR_TESLA);
    let resonse = await fetch(apiKey);
    let data = await resonse.json();
    setMyNews(data.articles ? data.articles.slice(0, 15) : []);
  };
